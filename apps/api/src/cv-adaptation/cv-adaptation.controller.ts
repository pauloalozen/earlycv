...
export class CvAdaptationController {
  ...
  // AnalysisJob já processado no backend. Ownership é checada dentro do
  // service (job.userId === user.id); jamais aceita jobId de outro
  // usuário. Idempotente: chamável repetidamente (ex.: polling autenticado
  ...
  claimAnalysisJob(
    ...
    return this.cvAdaptationService.claimGuestAnalysisJob(user.id, jobId);
  }
