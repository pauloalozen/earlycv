...
test("ResendEmailDeliveryService returns providerMessageId from the response body on success", async () => {
  const service = new ResendEmailDeliveryService();
  const json = mock.fn(async () => ({ id: "email_abc123" }));
  const fetchMock = mock.method(globalThis, "fetch", async () => {
    return {
      ok: true,
      status: 200,
  ...
  try {
    ...
    assert.deepEqual(result, { providerMessageId: "email_abc123" });
  } finally {
    fetchMock.mock.restore();
  }
});

test("ResendEmailDeliveryService returns providerMessageId:null when the success body can't be parsed, without failing the send", async () => {
  const service = new ResendEmailDeliveryService();
  const json = mock.fn(async () => {
