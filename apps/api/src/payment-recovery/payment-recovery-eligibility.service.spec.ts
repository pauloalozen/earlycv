    ...
    planPurchase: {
      findMany: async ({ where }: { where?: Record<string, unknown> } = {}) => {
        ...
        return data.purchases.filter((purchase) => {
          ...
          if (
            where.originAction &&
            purchase.originAction !== where.originAction
          )
            return false;
...
test("buy_credits without posterior credit eligible", async () => {
  const service = createService({
    purchases: [
      buildPurchase({ originAction: "buy_credits", originAdaptationId: null }),
    ],
  });
...
test("group history aggregates recovery emails across grouped purchases", async () => {
  ...
  assert.equal(
    result.items[0]?.lastRecoveryEmailSentAt,
