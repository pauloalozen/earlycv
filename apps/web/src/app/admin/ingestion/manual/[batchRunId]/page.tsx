...
import { getManualRunById, listManualRunItems } from "@/lib/admin-ingestion-api";
...
export default async function ManualRunDetailPage({ params }: ManualRunDetailPageProps) {
  const { batchRunId } = await params;
  const token = await getBackofficeSessionToken();

  if (!token) {
    const state = buildAdminStateModel("missing-token", `/admin/ingestion/manual/${batchRunId}`);
    return (
      <main className="min-h-screen bg-stone-50 px-6 py-10 text-stone-900 md:px-10">
        <Card className="mx-auto max-w-2xl space-y-4" padding="lg">
          ...
          <Link className={buttonVariants()} href={state.actionHref ?? "/admin/ingestion?tab=manual"}>
      ...
      <main className="min-h-screen bg-stone-50 px-6 py-10 text-stone-900 md:px-10">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h1 className="text-3xl font-bold tracking-tight">Execucao manual {run.id}</h1>
              <p className="text-sm text-stone-600">
                {run.scopeType} / {run.scopeValue} - status {run.status}
              </p>
            </div>
            <div className="flex gap-2">
              <Link className={buttonVariants({ variant: "outline" })} href="/admin/ingestion?tab=manual">
                Voltar
              </Link>
              {run.status === "queued" || run.status === "running" || run.status === "cancelling" ? (
                <form action={cancelManualRunAction}>
                  ...
                  <input name="redirectPath" type="hidden" value={`/admin/ingestion/manual/${run.id}`} />
              ...
              <tbody>
                {items.map((item) => (
                  <tr className="border-t border-stone-200" key={item.id}>
                    ...
                    <td className="px-4 py-3">{item.startedAt ? new Date(item.startedAt).toLocaleString("pt-BR") : "-"}</td>
                    <td className="px-4 py-3">{item.finishedAt ? new Date(item.finishedAt).toLocaleString("pt-BR") : "-"}</td>
