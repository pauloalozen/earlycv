...
// Fase 1.1 (Radar v2) — variante experimental de conversão, comparada lado a
// lado com RadarGuestAnalysisBand (radar/radar-guest-analysis-band.tsx, v1)
// numa rota separada (/radarv2/[slug]). Reaproveita 100% da lógica de
// negócio já validada em v1 (runRadarGuestAnalysisFlow, useTurnstileToken,
// analyzeGuestCv por trás dela) — só a apresentação muda: gauges reais
// (ScoreRing/scoreColor já usados no CompatCard logado, pra manter a MESMA
// linguagem visual entre guest e logado), estado de transição com
// checklist (Zeigarnik effect) e copy mais direta/curiosa.
const LOADING_STEPS = [
...
function LoadingState({ jobTitle }: { jobTitle: string }) {
  ...
  return (
    ...
    >
      <style>{`
        @keyframes radv2-spin { to { transform: rotate(360deg); } }
        @keyframes radv2-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.45; } }
      `}</style>
        ...
        >
          ...
          <circle
            ...
            style={{
              ...
              animation: "radv2-spin 1.1s linear infinite",
            }}
          />
        </svg>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                animation: isCurrent
                  ? "radv2-pulse 1.4s ease-in-out infinite"
                  : undefined,
              }}
            >
              {isDone ? (
...
export function RadarGuestAnalysisBandV2({
  jobId,
  jobTitle,
}: {
  jobId: string;
        ...
        >
          ...
          >
            ...
            <button
              ...
              onClick={() =>
                document
                  .getElementById("radarv2-guest-analysis-file-input")
                  ?.click()
              }
            ...
            <input
              id="radarv2-guest-analysis-file-input"
              type="file"
              accept=".pdf,.docx,.odt"
              className="hidden"
              onChange={(e) => {
