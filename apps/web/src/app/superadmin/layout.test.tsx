...
describe("SuperadminLayout access protection", () => {
  beforeEach(() => {
  ...
  it("calls notFound for anonymous users", async () => {
    ...
    await expect(SuperadminLayout({ children: <div /> })).rejects.toThrow(
      "NEXT_NOT_FOUND",
    );
  });
  ...
  it("calls notFound for staff admin users", async () => {
  ...
  it("allows staff superadmin users", async () => {
    ...
    const result = await SuperadminLayout({ children: <div /> });
